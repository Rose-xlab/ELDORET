...
export default function ScandalsAdminPage() {
  ...
  const fetchScandals = useCallback(async () => {
    try {
      const response = await fetch(`/api/scandals?page=${currentPage}`);
  ...
  useEffect(() => {
    fetchScandals();
  }, [fetchScandals, currentPage]); // Add currentPage as dependency
  ...
  const handleSearch = async () => {
    try {
      setLoading(true);
      setCurrentPage(1);
      const response = await fetch(`/api/scandals?userId=${searchId}&page=1`);
      const data = await response.json();
      setScandals(data.data);
      setTotalPages(data.pages);
    } catch (error) {
